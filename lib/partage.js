...
export function encodeBuild({ arche, niveau, stats, corps, slots, installations = [] }) {
  ...
  return '#/b/' + b64url([arche.id, niveau, deltas.join('.'), corpsTxt, emplacements, installations.join('.')].join('~'))
}
...
export function decodeBuild(hash) {
  ...
  try {
    ...
    const installations = (installationsTxt || '').split('.').filter(Boolean)
