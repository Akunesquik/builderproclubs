...
import ListePerks from '../molecule/composants/ListePerks.jsx'
 ...
 */
export default function PlaystylePanel({
  arche,
  stats,
  restant,
  deja,
  ...
  const traduirePlayStyle = (nom) => {
    const playstyle = DATA.playStyles.find(
      (p) => p.nom === nom
    )

    if (!playstyle) return nom
  ...
  const traduireCategorie = (categorie) => {
    return (
      t.playstyles?.categories?.[
        categorie
      ] ?? categorie
    )
  }

  const [hoveredPs, setHoveredPs] =
    useState(null)

  // Exemple : "Finisseur+" -> "Finisseur"
  const playStyleGold =
    spec?.archetypeGagne?.replace(
      /\+$/,
      ''
    )

  const liste = useMemo(() => {
    return playStylesAccessibles(
      arche
    ).map((ps) => ({
      ps,
      cout: coutPlayStyle(
