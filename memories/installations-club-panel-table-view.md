...
description: Modified the ClubFacilitiesPanel to display installations club data as a table with installations as rows, niveaux as columns, and bonuses in cells, with selectable cellules and total cost display. Also updated Bandeau to maintain compatibility with the new selections format.
---
...
To maintain compatibility with the parent component (Bandeau.jsx), the following updates were made:
- Bandeau now converts the installations prop from the old format (array of selected installation IDs) to the new format (object mapping IDs to niveau, defaulting to niveau 1) when passing to ClubFacilitiesPanel
- Bandeau also converts the onChange callback from the new format back to the old format (array of selected installation IDs) before calling the parent's onInstallations function
...
**How to apply:** The modified ClubFacilitiesPanel.jsx maintains backward compatibility through automatic format conversion in Bandeau.jsx. Other components that directly use ClubFacilitiesPanel may need similar updates to handle the new selections object format.